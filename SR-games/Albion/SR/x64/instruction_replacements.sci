...
loc_BAFEB,6,;mov ebp, [esp]|mov ebp, [r11d]|mov eax, 400|sub eax, [ebp-24h]|mov ebp, [self_mod_width]|mul ebp|mov edx, [loc_1414AC]|add edx, eax ; fix self-modifying (and length dependent) code
...
loc_C5EBB,4,;add esp, 4|add r11d, 4 ; fix esp instead of sp (probably mistake ???)

loc_94C79,6,mov [loc_13FFB0], esi|cmp dword [Game_UseEnhanced3DEngine], 0|jz _NonEnhanced3DEngine|;call draw_3dscene_proc|CALL draw_3dscene_proc|jmp _After3DEngine|_NonEnhanced3DEngine: ; call enhanced 3d engine
loc_94C84,5,_After3DEngine:|mov eax, [loc_14A4DA] ; label after 3d engine

loc_7C738,2,;mov ebp, esp|mov ebp, r11d|cmp ebx, 99|jbe loc_7C73A|cmp ebx, 255|ja loc_7C73A|mov ebx, 99|loc_7C73A: ; limit number of items to 99
...
loc_25156,1,;pop ebx|POP32 ebx|;call SR_Sync|CALL SR_Sync ; call sync after save
