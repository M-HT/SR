...
#else
        #define std__clamp(v,lo,hi) (((v) < (lo)) ? (lo) : ((hi) < (v)) ? (hi) : (v))
        s_m1(src + srcWidth * std__clamp(y - 1, 0, srcHeight - 1)),
        s_0 (src + srcWidth * std__clamp(y,     0, srcHeight - 1)),
        s_p1(src + srcWidth * std__clamp(y + 1, 0, srcHeight - 1)),
        s_p2(src + srcWidth * std__clamp(y + 2, 0, srcHeight - 1)),
#endif
        srcWidth_(srcWidth) {}
...
void xbrz::scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, ColorFormat colFmt, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
    if (factor == 1)
    {
        std::copy(src + yFirst * srcWidth, src + yLast * srcWidth, trg + yFirst * srcWidth);
        return;
    }
