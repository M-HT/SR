 ...
 * -----------------------------------------------------------------------------
 */
static void gen_operand(struct ud* u, struct ud_operand* op, int syn_cast, int ud_op_num, int ud_op_numops)
{
  switch(op->type) {
	case UD_OP_REG:
	...
	case UD_OP_IMM:
		...
		switch (op->size) {
			case  8: mkasm(u, "0x%x", ( (ud_op_num || ud_op_numops == 1)?op->lval.sbyte:op->lval.ubyte ) );    break;
...
extern void ud_translate_intel(struct ud* u)
{
  ...
  numops = (u->operand[2].type != UD_NONE)?3:( (u->operand[1].type != UD_NONE)?2:( (u->operand[0].type != UD_NONE)?1:0 ) );
  
  /* operand 1 */
  if (u->operand[0].type != UD_NONE) {
	gen_operand(u, &u->operand[0], u->c1, 0, numops);
  }
  /* operand 2 */
  if (u->operand[1].type != UD_NONE) {
	mkasm(u, ", ");
	gen_operand(u, &u->operand[1], u->c2, 1, numops);
  }

  /* operand 3 */
  if (u->operand[2].type != UD_NONE) {
	mkasm(u, ", ");
	gen_operand(u, &u->operand[2], u->c3, 2, numops);
  }
}
